...
u_int32_t get_if_addr(char *dev)
{
	...
	if (ioctl(skfd, SIOCGIFNETMASK, &ifr) < 0) {
		fprintf(stderr, "Couldn't get netmask of interface %s: error n° %d ", dev, errno);
		perror("");
		netmask = 0;
		return 0;
	}
...
void cleanup_on_exit()
{
	...
	sprintf(buf, "/sbin/rmmod nfnetlink_queue");
	system(buf);
...
int main(int argc, const char *argv[])
{
	...
	sprintf(buf, "sudo /sbin/modprobe nfnetlink_queue");
	if(system(buf) != 0) {
...
/* ./kpacman.ko port=%u */
	sprintf(buf, "sudo /sbin/insmod ./kernel/kpacman.ko port=%u",
...
//		exit(1);		
	...
	if(autoconf) set_if_addr(dev, ntohl(inet_addr("42.42.42.42")));
